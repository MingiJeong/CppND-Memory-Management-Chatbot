...
ChatLogic::ChatLogic()
{
    ...
    // TASK 5: due to duplicate Chatbot constructor, I commented out this.
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            // child and parent node as unique pointer; thus, it returns get() for actual managing object
                            // from the managing resource (after get), we can dereference by ->
                            // Finally, it approach the method (AddEge...) and take argument as (managing resource: address -- parent)
                            // or transfer edge ownership in case of child (unique pointer).
    ...
    ChatBot chatbot_local("../images/chatbot.png"); // instantiation
